        ...
        ULONG *pcFetched)   // actual count passed returned here
{
    ...
    while (cRealPins && (m_PinCount - m_Position)) {
		...
		if(std::find(m_PinCache.begin(), m_PinCache.end(), pPin) == m_PinCache.end())
        {
...
HRESULT CBaseOutputPin::DecideBufferSize(IMemAllocator* pAlloc, ALLOCATOR_PROPERTIES* ppropInputRequest)
{
	HRESULT hr;

	ppropInputRequest->cbBuffer = 0;
	ppropInputRequest->cBuffers = 0;
	ppropInputRequest->cbAlign = 1;
	ppropInputRequest->cbPrefix = 0;

	hr = GetBufferSize(ppropInputRequest);
...
HRESULT CDynamicOutputPin::ChangeMediaType(const CMediaType *pmt)
{
    ...
    /*  First check if the downstream pin will accept a dynamic
        format change
    */
    CComPtr<IPinConnection> pConnection;
    ...
    *ppBuffer = NULL;
    for (;;)
    {
        {  // scope for lock
			...
			if(!m_lFree.empty())
			{
	            pSample = (CMediaSample*)m_lFree.front();
...
CBaseAllocator::ReleaseBuffer(IMediaSample * pSample)
{
    ...
    BOOL bRelease = FALSE;
    {
		...
		m_lFree.push_front((CMediaSample*)pSample); // front because it's better for the cache to reuse buffers
    ...
    )
{
    ASSERT(m_fEnableReleaseCallback);
    CAutoLock cObjectLock(this);
    *plBuffersFree = m_lCount - m_lAllocated + m_lFree.size();
    return NOERROR;
}
...
CBaseAllocator::Alloc(void)
{
    ...
    ASSERT(m_lFree.size() == m_lAllocated);
...
CMemAllocator::ReallyFree(void)
{
    ...
    ASSERT(m_lAllocated == m_lFree.size());

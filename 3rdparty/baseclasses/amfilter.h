...
class CBaseMediaFilter : public CUnknown,
                                     public IMediaFilter
{
...
class CBaseFilter : public CUnknown,        // Handles an IUnknown
...
class CEnumPins : public IEnumPins      // The interface we support
{
	...
	std::list<CBasePin*> m_PinCache;	    // These pointers have not been AddRef'ed and
...
public:
	...
	virtual HRESULT GetBufferSize(ALLOCATOR_PROPERTIES* pProperties) PURE;
...
class CBaseAllocator : public CUnknown,// A non delegating IUnknown
                       public IMemAllocatorCallbackTemp, // The interface we support
                       public CCritSec             // Provides object locking
{
protected:

    std::list<CMediaSample*> m_lFree;        // Free list

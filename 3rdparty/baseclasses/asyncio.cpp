...
CAsyncIo::~CAsyncIo()
{
	...
	std::for_each(m_listDone.begin(), m_listDone.end(), [] (CAsyncRequest* r) {delete r;});
...
CAsyncIo::GetDoneItem()
{
	...
	if(!m_listDone.empty())
	{
		preq = m_listDone.front();

		m_listDone.pop_front();
	}

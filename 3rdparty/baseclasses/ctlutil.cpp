...
CCmdQueue::~CCmdQueue()
{
	...
	std::for_each(m_listPresentation.begin(), m_listPresentation.end(), [] (CDeferredCommand* cmd) {cmd->Release();});
	std::for_each(m_listStream.begin(), m_listStream.end(), [] (CDeferredCommand* cmd) {cmd->Release();});

	m_listPresentation.clear();
	m_listStream.clear();
...
CCmdQueue::Insert(CDeferredCommand* pCmd)
{
	...
	std::list<CDeferredCommand*>* pList = pCmd->IsStreamTime() ? &m_listStream : &m_listPresentation;

	std::list<CDeferredCommand*>::iterator i = pList->begin();

    // seek past all items that are before us

	while(i != pList->end() && (*i)->GetTime() <= pCmd->GetTime())
	{
		i++;
	}
...
CCmdQueue::Remove(CDeferredCommand* pCmd)
{
	...
	list<CDeferredCommand*>* pList = pCmd->IsStreamTime() ? &m_listStream : &m_listPresentation;
...
CCmdQueue::SetTimeAdvise(void)
{
    ...
    if(!m_listPresentation.empty())
	{
		current = m_listPresentation.front()->GetTime();
    }

    // if we're running, check the stream times too

    if(m_bRunning && !m_listStream.empty())
	{
	    // add on stream time offset to get presentation time

		CRefTime t = m_listPresentation.front()->GetTime() + m_StreamTimeOffset;
...
CCmdQueue::GetDueCommand(CDeferredCommand ** ppCmd, long msTimeout)
{
...

	{
	    ...
	    if (!m_listPresentation.empty()) {
			pCmd = m_listPresentation.front();
	    }

	    if (m_bRunning && !m_listStream.empty()) {
		CDeferredCommand* pStrm = m_listStream.front();

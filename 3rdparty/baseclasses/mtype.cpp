...
REFERENCE_TIME CMediaType::GetAvgTimePerFrame(REFERENCE_TIME atpf_default) const
{
	REFERENCE_TIME atpf = 0;

	if(formattype == FORMAT_VideoInfo)
	{
		atpf = ((const VIDEOINFOHEADER*)pbFormat)->AvgTimePerFrame;
	}
	else if(formattype == FORMAT_VideoInfo2)
	{
		atpf = ((const VIDEOINFOHEADER2*)pbFormat)->AvgTimePerFrame;
	}
	else if(formattype == FORMAT_MPEGVideo)
	{
		atpf = ((const MPEG1VIDEOINFO*)pbFormat)->hdr.AvgTimePerFrame;
	}
	else if(formattype == FORMAT_MPEG2Video)
	{
		atpf = ((const MPEG2VIDEOINFO*)pbFormat)->hdr.AvgTimePerFrame;;
	}

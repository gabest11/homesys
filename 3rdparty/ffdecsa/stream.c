...
trasp64_128_88ccw(sb);
  ...
  for(i=0;i<8;i++){
    for(b=0;b<4;b++){
      ctx->A[aboff+i][b]=iA[i][b];
      ctx->B[aboff+i][b]=iB[i][b];
    }
  }
  for(b=0;b<4;b++){
    ctx->A[aboff+8][b]=FF0();
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for(i=0;i<8;i++){
    ...
    for(j=0; j<4; j++){
      ...
      fe=ctx->A[aboff+3][0];fa=ctx->A[aboff+0][2];fb=ctx->A[aboff+5][1];fc=ctx->A[aboff+6][3];fd=ctx->A[aboff+8][0];
...
/* 0101 0101  1001 0011   : lev  5: */ tmp3=FFXOR(FFAND(fa,fc),FFXOR(fa,FFOR(FFAND(fa,fb),fd)));
      ...
      fe=ctx->A[aboff+1][1];fa=ctx->A[aboff+2][2];fb=ctx->A[aboff+5][3];fc=ctx->A[aboff+6][0];fd=ctx->A[aboff+8][1];
...
/* 0001 1110  1111 0101   : lev  5: */ tmp3=FFOR(FFAND(fa,fd),FFXOR(fa,FFXOR(fb,FFAND(fc,fd))));
      ...
      fe=ctx->A[aboff+0][3];fa=ctx->A[aboff+1][0];fb=ctx->A[aboff+4][1];fc=ctx->A[aboff+4][3];fd=ctx->A[aboff+5][2];
...
/* 1111 1111  1111 1111   : lev  0: */ tmp3=FF1();
      ...
      fe=ctx->A[aboff+2][3];fa=ctx->A[aboff+0][1];fb=ctx->A[aboff+1][3];fc=ctx->A[aboff+3][2];fd=ctx->A[aboff+7][0];
...
/* 1111 1111  1111 1111   : lev  0: */ tmp3=FF1();
      ...
      fe=ctx->A[aboff+4][2];fa=ctx->A[aboff+3][3];fb=ctx->A[aboff+5][0];fc=ctx->A[aboff+7][1];fd=ctx->A[aboff+8][2];
...
/* 0101 1101  1101 0101   : lev  4: */ tmp3=FFOR(FFAND(FFXOR(fa,fb),FFXOR(fc,FF1())),fd);
      ...
      fe=ctx->A[aboff+2][1];fa=ctx->A[aboff+3][1];fb=ctx->A[aboff+4][0];fc=ctx->A[aboff+6][2];fd=ctx->A[aboff+8][3];
...
/* 0001 0011  0000 0001   : lev  5: */ tmp3=FFAND(fc,FFXOR(FFAND(fa,FFXOR(fb,fd)),FFOR(fb,fd)));
      ...
      fe=ctx->A[aboff+1][2];fa=ctx->A[aboff+2][0];fb=ctx->A[aboff+6][1];fc=ctx->A[aboff+7][2];fd=ctx->A[aboff+7][3];
...
*/
      ...
      extra_B[3]=FFXOR(FFXOR(FFXOR(ctx->B[aboff+2][0],ctx->B[aboff+5][1]),ctx->B[aboff+6][2]),ctx->B[aboff+8][3]);
      extra_B[2]=FFXOR(FFXOR(FFXOR(ctx->B[aboff+5][0],ctx->B[aboff+7][1]),ctx->B[aboff+2][3]),ctx->B[aboff+3][2]);
      extra_B[1]=FFXOR(FFXOR(FFXOR(ctx->B[aboff+4][3],ctx->B[aboff+7][2]),ctx->B[aboff+3][0]),ctx->B[aboff+4][1]);
      extra_B[0]=FFXOR(FFXOR(FFXOR(ctx->B[aboff+8][2],ctx->B[aboff+5][3]),ctx->B[aboff+2][1]),ctx->B[aboff+7][0]);

      // T1 = xor all inputs
      // in1, in2, D are only used in T1 during initialisation, not generation
      for(b=0;b<4;b++){
        ctx->A[aboff-1][b]=FFXOR(ctx->A[aboff+9][b],ctx->X[b]);
      }

#ifdef STREAM_INIT
      for(b=0;b<4;b++){
        ctx->A[aboff-1][b]=FFXOR(FFXOR(ctx->A[aboff-1][b],ctx->D[b]),((j % 2) ? in2[b] : in1[b]));
      }
#endif
      ...
      for(b=0;b<4;b++){
        ctx->B[aboff-1][b]=FFXOR(FFXOR(ctx->B[aboff+6][b],ctx->B[aboff+9][b]),ctx->Y[b]);
      }

#ifdef STREAM_INIT
      for(b=0;b<4;b++){
        ctx->B[aboff-1][b]=FFXOR(ctx->B[aboff-1][b],((j % 2) ? in1[b] : in2[b]));
      }
#endif

      // if p=1, rotate left (yes, this is what we're doing)
      tmp3=ctx->B[aboff-1][3];
      ctx->B[aboff-1][3]=FFXOR(ctx->B[aboff-1][3],FFAND(FFXOR(ctx->B[aboff-1][3],ctx->B[aboff-1][2]),ctx->p));
      ctx->B[aboff-1][2]=FFXOR(ctx->B[aboff-1][2],FFAND(FFXOR(ctx->B[aboff-1][2],ctx->B[aboff-1][1]),ctx->p));
      ctx->B[aboff-1][1]=FFXOR(ctx->B[aboff-1][1],FFAND(FFXOR(ctx->B[aboff-1][1],ctx->B[aboff-1][0]),ctx->p));
      ctx->B[aboff-1][0]=FFXOR(ctx->B[aboff-1][0],FFAND(FFXOR(ctx->B[aboff-1][0],tmp3),ctx->p));

      // T3 = xor all inputs
      for(b=0;b<4;b++){
        ctx->D[b]=FFXOR(FFXOR(ctx->E[b],ctx->Z[b]),extra_B[b]);
      }
      ...
      tmp0=FFXOR(ctx->Z[0],ctx->E[0]);
      tmp1=FFAND(ctx->Z[0],ctx->E[0]);
      ctx->F[0]=FFXOR(ctx->E[0],FFAND(ctx->q,FFXOR(ctx->Z[0],ctx->r)));
      tmp3=FFAND(tmp0,ctx->r);
      tmp4=FFOR(tmp1,tmp3);

      tmp0=FFXOR(ctx->Z[1],ctx->E[1]);
      tmp1=FFAND(ctx->Z[1],ctx->E[1]);
      ctx->F[1]=FFXOR(ctx->E[1],FFAND(ctx->q,FFXOR(ctx->Z[1],tmp4)));
      tmp3=FFAND(tmp0,tmp4);
      tmp4=FFOR(tmp1,tmp3);

      tmp0=FFXOR(ctx->Z[2],ctx->E[2]);
      tmp1=FFAND(ctx->Z[2],ctx->E[2]);
      ctx->F[2]=FFXOR(ctx->E[2],FFAND(ctx->q,FFXOR(ctx->Z[2],tmp4)));
      tmp3=FFAND(tmp0,tmp4);
      tmp4=FFOR(tmp1,tmp3);

      tmp0=FFXOR(ctx->Z[3],ctx->E[3]);
      tmp1=FFAND(ctx->Z[3],ctx->E[3]);
      ctx->F[3]=FFXOR(ctx->E[3],FFAND(ctx->q,FFXOR(ctx->Z[3],tmp4)));
      tmp3=FFAND(tmp0,tmp4);
      ctx->r=FFXOR(ctx->r,FFAND(ctx->q,FFXOR(FFOR(tmp1,tmp3),ctx->r))); // ultimate carry
...
*/
      ...
      // xor 2 by 2
      cb_g[8*i+7-2*j]=FFXOR(ctx->D[2],ctx->D[3]);
      cb_g[8*i+6-2*j]=FFXOR(ctx->D[0],ctx->D[1]);
#endif
  ...
  for(k=0;k<10;k++){
    for(b=0;b<4;b++){
      ctx->A[32+k][b]=ctx->A[k][b];
      ctx->B[32+k][b]=ctx->B[k][b];
    }
  }

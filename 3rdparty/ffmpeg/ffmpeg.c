...
int IsVista()
{
	...
	return GetVersionEx(&osver) && osver.dwPlatformId == VER_PLATFORM_WIN32_NT && osver.dwMajorVersion >= 6 ? 1 : 0;
}
...
void FFH264DecodeBuffer(struct AVCodecContext* pAVCtx, BYTE* pBuffer, UINT nSize, int* pFramePOC, int* pOutPOC, REFERENCE_TIME* pOutrtStart, REFERENCE_TIME* pOutrtStop)
{
	if(pBuffer != NULL)
	{
		...
		av_h264_decode_frame(pAVCtx, pOutPOC, pOutrtStart, pOutrtStop, pBuffer, nSize);

		if(h->s.current_picture_ptr && pFramePOC) 
		{
...
int FFH264CheckCompatibility(int nWidth, int nHeight, struct AVCodecContext* pAVCtx, BYTE* pBuffer, UINT nSize, int nPCIVendor, LARGE_INTEGER VideoDriverVersion)
{
	...
	if (pBuffer != NULL)
		av_h264_decode_frame (pAVCtx, NULL, NULL, NULL, pBuffer, nSize);
...
USHORT FFH264FindRefFrameIndex(USHORT num_frame, DXVA_PicParams_H264* pDXVAPicParams, int vendor)
{
	int		i;
	for (i=0; i<pDXVAPicParams->num_ref_frames; i++)
	{
		if (pDXVAPicParams->FrameNumList[i] == num_frame)
			return vendor == PCIV_Intel ? (USHORT)i : pDXVAPicParams->RefFrameList[i].Index7Bits;
	}
...
HRESULT FFH264BuildPicParams (DXVA_PicParams_H264* pDXVAPicParams, DXVA_Qmatrix_H264* pDXVAScalingMatrix, int* nFieldType, int* nSliceType, struct AVCodecContext* pAVCtx, int nPCIVendor)
{
	...
	//	pDXVAPicParams->SliceGroupMap
		...
		pDXVAPicParams->log2_max_pic_order_cnt_lsb_minus4		= cur_sps->poc_type == 0 ? cur_sps->log2_max_poc_lsb - 4 : 0; // log2_max_pic_order_cnt_lsb_minus4;
		...
		else
		{
			...
			pDXVAPicParams->CurrFieldOrderCnt[0]	= h->poc_lsb + h->poc_msb;
			pDXVAPicParams->CurrFieldOrderCnt[1]	= h->poc_lsb + h->poc_msb + h->delta_poc_bottom;
		}
...
void FF264UpdateRefFrameSliceLong(DXVA_PicParams_H264* pDXVAPicParams, DXVA_Slice_H264_Long* pSlice, struct AVCodecContext* pAVCtx, int vendor)
{
	...
	memset(pSlice->RefPicList, 0xff, sizeof(pSlice->RefPicList));
	...
	if(h->slice_type != FF_I_TYPE && h->slice_type != FF_SI_TYPE) 
	{
		if(h->ref_count[0] > 0)
		{
			...
			{
				pSlice->RefPicList[0][i].Index7Bits = FFH264FindRefFrameIndex(h->ref_list[0][i].frame_num, pDXVAPicParams, vendor);
	...
	if(h->slice_type == FF_B_TYPE || h->slice_type == FF_S_TYPE || h->slice_type == FF_BI_TYPE) 
	{
		if(h->ref_count[1] > 0)
		{
			...
			{
				pSlice->RefPicList[1][i].Index7Bits = FFH264FindRefFrameIndex(h->ref_list[1][i].frame_num, pDXVAPicParams, vendor);
	...
	if(h->slice_type == FF_I_TYPE || h->slice_type == FF_SI_TYPE 
	|| h->slice_type == FF_P_TYPE || h->slice_type == FF_SP_TYPE) 
	{	
		memset(pSlice->RefPicList[1], 0xff, sizeof(pSlice->RefPicList[1]));

		if(h->slice_type == FF_I_TYPE || h->slice_type == FF_SI_TYPE) 
		{
			memset(pSlice->RefPicList[0], 0xff, sizeof(pSlice->RefPicList[0]));
		}
	} 
}
...
HRESULT FFVC1UpdatePictureParam(DXVA_PictureParameters* pPicParams, struct AVCodecContext* pAVCtx, int* nFieldType, int* nSliceType, BYTE* pBuffer, UINT nSize)
{
	...
	*nSliceType = vc1->s.pict_type;
	...
	// Cf ｧ7.1.1.25 in VC1 specification, ｧ3.2.14.3 in DXVA spec
...
//	pPicParams->bSecondField;
	...
	*nSliceType = s->current_picture.pict_type;

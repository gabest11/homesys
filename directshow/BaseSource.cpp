...
HRESULT CBaseStream::SetRate(double rate)
{
	if(rate <= 0)
		return E_INVALIDARG;
...
HRESULT CBaseStream::FillBuffer(IMediaSample* pSample)
{
...

	{
		...
		int frame = (int)(m_rtPosition / m_AvgTimePerFrame); // (int)(1.0 * m_rtPosition / m_AvgTimePerFrame + 0.5);

		long len = pSample->GetSize();

		hr = FillBuffer(pSample, frame, pOut, len);

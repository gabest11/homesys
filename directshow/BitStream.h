...
interface IBitStream : public IUnknown
{
	...
	STDMETHOD(ByteWrite)(const void* buff, int len) = 0;
	STDMETHOD(BitWrite)(UINT64 data, int len) = 0;
	STDMETHOD(BitFlush)() = 0;
	STDMETHOD(StrWrite)(LPCSTR str, BOOL bFixNewLine) = 0;
};
...
public:
	CBitStream(IStream* stream, bool m_fThrowError = false);
	virtual ~CBitStream();
	...
	STDMETHODIMP ByteWrite(const void* buff, int len);
	STDMETHODIMP BitWrite(UINT64 data, int len);
	STDMETHODIMP BitFlush();
	STDMETHODIMP StrWrite(LPCSTR str, BOOL bFixNewLine);
};

...
#define MAX_VIA_PROV_COUNT 30 
 
typedef struct   
{ 
	U8					    auPrgInfo[MAX_VIA_PROV_COUNT][3]; 
	U8					    auSA[MAX_VIA_PROV_COUNT][4];	 
	int						iPrgCount; 
	int						iPrgIndex; 
	U32						uCardNumber; 
	int						iAgeGrade; 
	U32						uPPUA; 
	U8						uCardUA[5]; 
	unsigned char			ucViaSCCustWp; 
}Viaccess_Info_t; 
 
 
static Viaccess_Info_t  viaccess; /*畠蕉延楚・壓崘嬬触兜兵晒朔辛參岷俊聞喘万議匯乂佚連阻*/ 
...
bool via_parse_emm(Smart_Handle_t Handle,U8* pbuf,U16 len); 

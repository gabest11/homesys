...
interface IDSMPropertyBag : public IPropertyBag2
{
	...
	STDMETHOD(RemoveAt)(LPCWSTR key) = 0;
};
...
public:
	...
	STDMETHODIMP RemoveAt(LPCWSTR key);
};
...
public:
	...
	CDSMResource(LPCWSTR name, LPCWSTR desc, LPCWSTR mime, BYTE* data, int len, DWORD_PTR tag = 0);
	virtual ~CDSMResource();

	// global access to all resources

	static CCritSec m_csResources;
	static std::map<DWORD_PTR, CDSMResource*> m_resources;
};
...
public:
	...
	void operator += (const CDSMChapter& c) {m_chapters.push_back(c); m_sorted = false;}
...
template<class T>
int range_bsearch(const std::vector<T>& a, REFERENCE_TIME rt)
{
	int i = 0, j = a.size() - 1, ret = -1;
	...
	while(i < j)
	{
		...
		REFERENCE_TIME midrt = a[mid].m_rt;

...
HRESULT CBinary::Write(IStream* pStream)
{
	...
	HeaderWrite(pStream);
	return pStream->Write(data(), size(), NULL);
}
...
HRESULT CANSI::Write(IStream* pStream)
{
	...
	HeaderWrite(pStream);
	return pStream->Write(c_str(), size(), NULL);
}
...
UINT64 CUTF8::Size(bool fWithHeader)
{
	...
	UINT64 len = Util::UTF16To8(c_str()).size();
	if(fWithHeader) len += HeaderSize(len);
	return len;
}
...
HRESULT CUTF8::Write(IStream* pStream)
{
	...
	HeaderWrite(pStream);
	std::string s = Util::UTF16To8(c_str());
	return pStream->Write(s.c_str(), s.size(), NULL);
}
...
{
	if(m_len == 0x00FFFFFFFFFFFFFFi64) return 8;
...
{
	UINT64 len = TrackNumber.Size() + 2 + 1; // TrackNumber + TimeCode + Lacing

	if(BlockData.size() > 1)
	{
		len += 1; // nBlockData

		for(auto i = BlockData.begin(); i != BlockData.end(); )
		{
			CBinary* b = *i++;
...
HRESULT CBlock::Write(IStream* pStream)
{
	...
	for(auto i = BlockData.begin(); i != BlockData.end(); i++)
	{
		...
		pStream->Write(b->data(), b->size(), NULL);
	}
...
HRESULT Void::Write(IStream* pStream)
{
	...
	{
		pStream->Write(buff, (ULONG)std::min<int>(len, sizeof(buff)), NULL);
	}

...
// CNullRawVideoRenderer

CNullRawVideoRenderer::CNullRawVideoRenderer(LPUNKNOWN pUnk, HRESULT* phr) 
	: CNullRenderer(__uuidof(this), NAME("Null Video Renderer (Uncompressed)"), pUnk, phr)
{
}

HRESULT CNullRawVideoRenderer::CheckMediaType(const CMediaType* pmt)
{
	return pmt->majortype == MEDIATYPE_Video
...
// CNullRawAudioRenderer

CNullRawAudioRenderer::CNullRawAudioRenderer(LPUNKNOWN pUnk, HRESULT* phr) 
	: CNullRenderer(__uuidof(this), NAME("Null Audio Renderer (Uncompressed)"), pUnk, phr)
{
}

HRESULT CNullRawAudioRenderer::CheckMediaType(const CMediaType* pmt)
{
	return pmt->majortype == MEDIATYPE_Audio
...
{
	m_pInput = new CTextInputPin(this, this, phr);
}

CNullTextRenderer::~CNullTextRenderer()
{
	delete m_pInput;
}

int CNullTextRenderer::GetPinCount() 
{
	return m_pInput != NULL ? 1 : 0;
}

CBasePin* CNullTextRenderer::GetPin(int n) 
{
	return n == 0 ? (CBasePin*)m_pInput : NULL;
}

CNullTextRenderer::CTextInputPin::CTextInputPin(CBaseFilter* pFilter, CCritSec* pLock, HRESULT* phr) 

...
inline unsigned int VDRevBits32(unsigned int x) {
	unsigned int y;
...
inline unsigned int VDRevBits(unsigned int v, unsigned bits) {
...
void VDMakePermuteTable(unsigned int *dst0, unsigned bits) {
	...
	ASSERT((unsigned int)(dst - dst0) == N - (1 << ((bits + 1) >> 1)));
}
...
void VDCreateRaisedCosineWindow(float *dst, int n) {
	const double twopi_over_n = (float)(M_PI * 2) / n;
...
void VDComputeRealFFT(float *p, unsigned bits) {
	...
	float *p0 = p + 2;
	float *p1 = p + N - 2;
	float angstep = (float)M_PI / (float)(1 << bits) * 2.0f;
...
void VDComputeComplexFFT_Reference(float *out, float *in, unsigned bits, double sign) {
	unsigned N = 1 << bits;
	double angstep = sign*2.0*(float)M_PI / (double)N;
...
void VDRealFFT::Init(unsigned bits) {
	...
	mpPermuteTable = new unsigned int[N2];
...
void VDRollingRealFFT::Init(unsigned bits) {
	...
	mpWindow = (float *)_aligned_malloc(mPoints*sizeof(float), 16);

	const float step = 2*(float)M_PI / (int)mPoints;
	const float scalefac = 0.5f / mPoints;

	for(unsigned int i=0; i<mPoints; ++i)
		mpWindow[i] = scalefac * (1.0f - cosf(step * ((int)i+0.5f)));

	mpSampleBuffer = (float *)_aligned_malloc(mPoints*sizeof(float), 16);

	mpTempArea = (float *)_aligned_malloc(mPoints*sizeof(float), 16);
...
void VDRollingRealFFT::Shutdown() {
	if (mpWindow) {
		_aligned_free(mpWindow);
		mpWindow = NULL;
	}

	if (mpTempArea) {
		_aligned_free(mpTempArea);
		mpTempArea = NULL;
	}

	if (mpSampleBuffer) {
		_aligned_free(mpSampleBuffer);
		mpSampleBuffer = NULL;
	}
...
void VDRollingRealFFT::Advance(unsigned int samples) {
...
void VDRollingRealFFT::CopyIn8U(const unsigned char *src, unsigned int count, ptrdiff_t stride) {
	if (count <= 0)
		return;
	...
	mBufferLevel += count;
	do {
		*dst++ = ((int)*src - 0x80) * (1.0f / 128.0f);
		src += stride;
	} while(--count);
}

void VDRollingRealFFT::CopyIn16S(const short *src, unsigned int count, ptrdiff_t stride) {
	if (count <= 0)
		return;

	if (count > mPoints) {
		src = (const short *)((const char *)src + (count-mPoints)*stride);
		count = mPoints;
	}

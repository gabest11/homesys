...
// Based on Intel's SSE2 implementation of iDCT
// AP-945
...
void mpeg2_idct_copy_sse2(int16_t* RESTRICT block, uint8_t* RESTRICT dest, const int stride)
{
	idct_M128ASM(block);
/*
    for(int i = 0; i < 8; i++)
...
*/
	Vector4i* src = (Vector4i*)block;

	Vector4i::store(&dest[stride * 0], &dest[stride * 1], src[0].pu16(src[1]));
	Vector4i::store(&dest[stride * 2], &dest[stride * 3], src[2].pu16(src[3]));

	dest += stride * 4;

	Vector4i::store(&dest[stride * 0], &dest[stride * 1], src[4].pu16(src[5]));
	Vector4i::store(&dest[stride * 2], &dest[stride * 3], src[6].pu16(src[7]));
...
void mpeg2_idct_add_sse2(const int last, int16_t* RESTRICT block, uint8_t* RESTRICT dest, const int stride)
{
	idct_M128ASM(block);
	...
	*/
	...
	Vector4i v0, v1, v2, v3;

	v0 = src[0].adds16(Vector4i::loadl(&dest[stride * 0]).upl8());
	v1 = src[1].adds16(Vector4i::loadl(&dest[stride * 1]).upl8());
	v2 = src[2].adds16(Vector4i::loadl(&dest[stride * 2]).upl8());
	v3 = src[3].adds16(Vector4i::loadl(&dest[stride * 3]).upl8());

	Vector4i::store(&dest[stride * 0], &dest[stride * 1], v0.pu16(v1));
	Vector4i::store(&dest[stride * 2], &dest[stride * 3], v2.pu16(v3));

	dest += stride * 4;

	v0 = src[4].adds16(Vector4i::loadl(&dest[stride * 0]).upl8());
	v1 = src[5].adds16(Vector4i::loadl(&dest[stride * 1]).upl8());
	v2 = src[6].adds16(Vector4i::loadl(&dest[stride * 2]).upl8());
	v3 = src[7].adds16(Vector4i::loadl(&dest[stride * 3]).upl8());

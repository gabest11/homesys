...
MediaChunk::~MediaChunk()
{
    ...
    while(m_Sampless.size() > 0)
    {
        delete m_Sampless.front();
        m_Sampless.pop_front();
    }
}
...
{
	MediaSample* s = new MediaSample();

	s->sync = pSample->IsSyncPoint() == S_OK;
	...
	int bytes = pSample->GetActualDataLength();

    m_cBytes += bytes;

	s->size = bytes;
	s->buff = new unsigned char[bytes];

	BYTE* buff = NULL;

	if(FAILED(pSample->GetPointer(&buff))) return E_FAIL;

	memcpy(s->buff, buff, s->size);

    m_Sampless.push_back(s);
...
MediaChunk::Write(Atom* patm)
{
	...
	for (auto it = m_Sampless.begin(); it != m_Sampless.end(); it++)
    {
        MediaSample* s = *it;
		...
		int cActual = 0;
		m_pTrack->Handler()->WriteData(patm, s->buff, s->size, &cActual);
		cBytes += cActual;

		if(s->hastime)
        {
			// this is the last buffer in the sample
			m_pTrack->IndexSample(bSync, s->start, s->stop, cBytes);

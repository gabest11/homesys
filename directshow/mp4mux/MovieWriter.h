...
public:
	MediaSample() : buff(NULL), size(0), sync(false), hastime(false) {}
	virtual ~MediaSample() {delete [] buff;}
};
...
private:
    ...
    long m_cBytes;
    std::list<MediaSample*> m_Sampless;
};
...
private:
    ...
    std::list<MediaChunkPtr> m_Queue;
...
private:
    ...
    std::vector<TrackWriterPtr> m_Tracks;
};

...
namespace DXUI
{
	Texture10::Texture10(ID3D10Texture2D* texture)
		: m_texture(texture)
	{
		ASSERT(m_texture);
		...
		else if(m_desc.BindFlags & D3D10_BIND_SHADER_RESOURCE) m_type = Video;
		else if(m_desc.Usage == D3D10_USAGE_STAGING) m_type = System;
	...
	bool Texture10::Update(const Vector4i& r, const void* data, int pitch)
	{
		if(m_dev && m_texture)
		{
	...
	bool Texture10::Map(MemMap& m, const Vector4i* r)
	{
		if(r != NULL)
		{
	...
	bool Texture10::Save(LPCWSTR fn, bool dds)
	{
	...
	Texture10::operator ID3D10Texture2D*()
	{
		return m_texture;
	}

	Texture10::operator ID3D10ShaderResourceView*()
	{
	...
	Texture10::operator ID3D10RenderTargetView*()
	{
		ASSERT(m_dev);
	...
	Texture10::operator ID3D10DepthStencilView*()
	{

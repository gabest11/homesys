...
namespace DXUI
{
	Texture11::Texture11(ID3D11Texture2D* texture)
		: m_texture(texture)
	{
		ASSERT(m_texture);
		...
		else if(m_desc.BindFlags & D3D11_BIND_SHADER_RESOURCE) m_type = Video;
		else if(m_desc.Usage == D3D11_USAGE_STAGING) m_type = System;
	...
	bool Texture11::Update(const Vector4i& r, const void* data, int pitch)
	{
		if(m_dev && m_texture)
		{
	...
	bool Texture11::Map(MemMap& m, const Vector4i* r)
	{
		if(r != NULL)
		{
	...
	bool Texture11::Save(LPCWSTR fn, bool dds)
	{
		...
		if(m_desc.BindFlags & D3D11_BIND_DEPTH_STENCIL)
		{
			...
			for(DWORD y = 0; y < desc.Height; y++, s += sm.RowPitch, d += dm.RowPitch)
			{
				for(DWORD x = 0; x < desc.Width; x++)
				{
					((DWORD*)d)[x] = (DWORD)(((float*)s)[x*2] * UINT_MAX);
				}
			}
		...
		return SUCCEEDED(D3DX11SaveTextureToFile(m_ctx, res, dds ? D3DX11_IFF_DDS : D3DX11_IFF_BMP, fn));
	}

	Texture11::operator ID3D11Texture2D*()
	{
		return m_texture;
	}

	Texture11::operator ID3D11ShaderResourceView*()
	{
	...
	Texture11::operator ID3D11RenderTargetView*()
	{
		ASSERT(m_dev);
	...
	Texture11::operator ID3D11DepthStencilView*()
	{
